(()=>{
  ...
  function render(){
    ...
    const signature=JSON.stringify([room.id,phase,on,camera,microphone,remote,connectionState,providerError,lastPresenceKey,devices.audioinput.map(item=>item.deviceId),devices.videoinput.map(item=>item.deviceId)]);if(section.dataset.renderSignature===signature)return;section.dataset.renderSignature=signature;
