(()=>{
  ...
  let stream=null,access=null,busy=false,liveRoom=null,sdkPromise=null,renderFrame=null,mediaHeartbeat=null,presenceClock=null,lastPresenceKey='',devices={audioinput:[],videoinput:[]};
  ...
  async function startMedia(){
    ...
    try{
      ...
      await saveState({connection_state:'connected',camera_enabled:cameraEnabled(),microphone_enabled:microphoneEnabled()});startMediaHeartbeat();await refreshDevices();window.DeviceManager?.haptic?.('success');render();
  ...
  async function stopMedia(report=true){stopMediaHeartbeat();stream?.getTracks?.().forEach(track=>track.stop());stream=null;if(liveRoom){liveRoom.disconnect();liveRoom=null}if(report)await saveState({connection_state:'idle',camera_enabled:false,microphone_enabled:false}).catch(()=>{});render()}
