(()=>{
  ...
  function renderCoverage(recipe){
    ...
    card.innerHTML=`<div style="padding:16px"><span class="eyebrow">Pantry coverage</span><h3 style="margin:6px 0">You already have ${coverage.available_count||0} of ${coverage.required_count||0} ingredients</h3><p style="margin:0 0 12px;color:var(--muted,#756f69)">${coverage.coverage_percent||0}% covered${soon.length?` · Uses ${soon.length} item${soon.length===1?'':'s'} soon`:''}</p><div style="display:grid;gap:10px"><div><strong>Already have</strong><div>${have.length?have.map(item=>`<span class="chip">${String(item)}</span>`).join(' '):'None matched'}</div></div><div><strong>Need to buy</strong><div id="pantryShoppingGap">${buy.length?buy.map(item=>`<span class="chip">${String(item)}</span>`).join(' '):'Nothing — your pantry covers this recipe.'}</div></div></div></div>`;
